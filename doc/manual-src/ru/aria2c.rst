...
Загрузка файла .torrent, без загрузки его содержимого
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
.. code-block:: console
...
В качестве альтернативы, когда PEM-файлы поддерживаются, указать файл с
сертификатом сервера и файл с закрытым ключом следующим образом:
