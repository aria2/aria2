...
ssize_t WinTLSSession::readData(void* data, size_t len)
{
  ...
  // Read as many bytes as available from the connection.
  ...
  while (readBuf_.size()) {
    ...
    if (status_ == SEC_I_CONTEXT_EXPIRED) {
      // Conenction is gone now, but the buffered bytes are still valid.
...
read:
      ...
      // Read as many bytes as possible. We do not know how many bytes will
      // arrive from the server at this point.
